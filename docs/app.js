...
class ConnectScreen {
  ...
  view() {
    return m("div", {
      class: "card"
    }, m("header", null, m("h3", null, "Connect")), m("input", {
      value: model.host,
      oninput: m.withAttr('value', v => model.host = v),
      onkeypress: e => Connect(e.charCode)
    }), m("footer", null, m("button", {
      onclick: () => model.connect(),
      class: model.isConnecting ? 'loading' : ''
    }, "Connect")));
  }
...
class ClientScreen {
  constructor() {
    if (!model.socket) m.route.set('/');
  }

  view() {
    return m("article", {
      class: "card"
    }, m("header", null, m("h3", null, "Client: ", model.status)), m('.listview', {
      onupdate: v => scrollToBottom(v)
    }, model.messages.map(msg => m('p.stack', msg))), m("input", {
      value: model.line,
      oninput: m.withAttr('value', v => model.line = v),
      onkeypress: e => sendLineOnKeyPress(e),
      autofocus: true
    }), m("footer", null, m("button", {
      onclick: () => sendLine()
    }, "Send"), m("button", {
